    ...
    <div class="d-grid">
        <?php foreach(AsideData::list() as $key => $item) { $icon = $item['icon']; ?>
            ...
            <div class="accordion position-relative">
                <a href="<?=$item['link'];?>" class="<?=$currentUrl === $item['link'] ? 'activelink ' : '';?>py-2 link accordion-button text-left d-flex align-items-center gap-3 text-decoration-none text-body-tertiary">
